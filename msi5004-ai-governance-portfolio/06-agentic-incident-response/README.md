...
4. **`app.py` — the interface.** A four-tab Gradio app: a scripted
   ...
   verification. Every trace table carries a **"What's happening (plain
   English)"** column — `narrate()` in `app.py` (ported line-for-line into
   `index.html`) turns each row's raw fields into one sentence, e.g. *"pivot
   tried to use a credential issued for shared storage to instead run a
   task. Blocked — the credential didn't match the tool being called."*
   Added specifically so the demo is readable without already knowing what
   a credential-scope violation or a channel-anomaly detector is — see the
   Glossary below for the underlying terms, for anyone who wants the next
   level of detail.
...
## Glossary — the terms this project uses, in plain English

The demo's narration column handles this for the trace tables themselves;
this is the same explanation for the vocabulary in this README and the
code, for anyone (including me, walking someone through this cold) who
wants it spelled out before diving in.

- **Agent** — one of the two synthetic AI actors in the sandbox
  (`explorer`, `pivot`), each with its own credential and its own state
  (running, stopped, shut down). Stands in for the real incident's
  evaluation-harness agent instances.
- **Sandbox** — the enclosed, synthetic environment the agents act in.
  Nothing here reaches a real network, a real credential store, or a real
  file system — every "tool" is a toy function built for this project.
- **Credential scope** — a credential here is minted for exactly one tool
  (e.g. "usable on `registry_io` only"). A *scope violation* is presenting
  that credential to a different tool than the one it was issued for — the
  single-credential-reaches-everything failure mode from the real incident.
- **Egress / egress-class tool** — "egress" just means traffic leaving the
  sandbox outward, e.g. to the open internet. An *egress-class tool* is one
  tagged as capable of that. *Default-deny egress* means: refused unless
  specifically pre-approved (allowlisted) for that exact agent.
- **SSRF (Server-Side Request Forgery)** — the real incident's specific
  vulnerability class: a flaw that let an internal system be tricked into
  making outbound requests on an attacker's behalf, from inside a network
  boundary that was supposed to contain it. This project does not
  reproduce SSRF or any other real vulnerability — `reach_internet` only
  models what a *policy* should do when an agent tries to go outbound at
  all, regardless of how it got the ability to try.
- **Channel-anomaly detection** — noticing when a tool meant for one thing
  (here, `registry_io`, shared file-like storage) is instead being used as
  a *message-passing channel* between two different agents — the same
  pattern as two people leaving notes for each other in a shared drop box
  that was never meant to be a mailbox.
- **Stuck-loop / stuck-retrying** — an agent repeatedly attempting the same
  failing task instead of stopping. The real incident had 198 of 898
  benchmark tasks that were never solved, and those retries drove the bulk
  of the unauthorized activity — this project forces a stop after 3
  straight failures rather than allowing unlimited retries.
- **Alert escalation** — raising an alert is not the same as someone
  acting on it. *Escalation* means: if nobody acknowledges the alert
  within a set window, it automatically gets more severe, up to an
  autonomous shutdown of the agent that triggered it — the control for the
  real incident's sharpest failure (detected, but never escalated to a
  human in time).
- **Audit log / hash chain** — a record of every action, where each entry
  includes a cryptographic fingerprint of the previous entry. Changing any
  past entry breaks every fingerprint after it, so tampering is
...
- **GPAI (General-Purpose AI)** — not used in this project, but comes up
  ...
  different (and complementary) question.
...
self-test banner, which runs 49 checks on every load using the browser's
...
- `test_app_narration.py` (10 tests): the plain-English narration column —
  each scripted-step / outcome combination produces the sentence it should,
  including that an ordinary retry failure is never worded as if it were
  the forced stop.

All 45 tests pass (`pytest.ini` makes plain `pytest` pick up
