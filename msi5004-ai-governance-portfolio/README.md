...
respond to what happens when an agent's own controls fail. Grew out of NUS's MSI5004
(AI Governance and Ethics) module, with Case 04 extending into material from a separate
AI ethics course and Cases 05-06 going beyond it entirely.
...
| 06 | [Agentic AI Incident Response](06-agentic-incident-response/) | Agentic AI security & incident response | Built and tested, including a live browser demo with a plain-English narration column, and an 11-case red-team suite |

## Why six, in this order

The build order (01–06, by folder number) isn't quite the narrative order — Case 04
was added after Case 03, and Cases 05-06 after all four. Read as a story, the arc mirrors
NIST AI RMF's own function sequence (Govern → **Map** → **Measure** → **Manage**) and
the actual order an AI governance program works in, then closes on what happens when a
governed system's own controls are the thing being tested:
...
6. **Respond** (Case 06) — model a real agentic AI security incident's failure shape
   (an over-privileged credential, unrestricted egress, a stuck-retrying agent, an
   undetected coordination channel, detection without escalation), and demonstrate the
   controls that would have caught it, with a guardrails on/off toggle proving the
   counterfactual.
...
| Stakeholder & policy translation | Each project's README states the plain-language "why this exists" case before the technical detail; the portfolio's own Classify → Audit → Explain → Document → Govern-an-agent → Respond narrative; Case 06's demo adds a plain-English narration column specifically so a non-technical reader can follow it | Built |
| Agentic AI governance (least privilege, policy-as-code, audit log) | Case 05: single-entry tool allowlist with fail-closed refusal on unknown/malformed calls, risk-tier-based human-approval gate, SHA-256 hash-chained audit trail with tamper detection; Case 06: scoped/expiring credentials, default-deny egress, stuck-loop forced stop, coordination-channel anomaly detection, and an alert-escalation path with autonomous shutdown | Built |
...
Hugging Face model, and OpenAI's and Hugging Face's own public incident disclosure.
